...
export function TeamRoster({
  ...
  return (
    <section className={twMerge(' border border-white/10 bg-neutral-900 p-5', className)}>
      <h3 className="mb-4 text-xs font-bold tracking-[0.18em] text-neutral-500 uppercase">
        {name}
      </h3>
      <ul className="grid gap-2">
        {players.map((player) => (
          <li key={player.id} className="flex items-center justify-between  bg-black/20 px-3 py-2">
