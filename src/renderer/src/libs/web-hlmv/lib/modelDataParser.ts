...
import { MultiArrayView } from './multiArrayView'

/**
 * Creates multiple reader
 * @internal
 */
const createMultipleParser =
  <S extends Struct>(struct: S) =>
  (dataView: DataView, offsetIndex: number, number: number): StructResult<S>[] =>
    BinaryReader.readStructMultiple(dataView, struct, offsetIndex, number)

/** Parses header of the MDL file */
export const parseHeader = (dataView: DataView): structs.Header =>
  BinaryReader.readStruct(dataView, structs.header)
...
export const parseSubModel = (
  dataView: DataView,
  bodyParts: structs.BodyPart[]
): structs.SubModel[][] =>
  bodyParts.map((bodyPart) =>
    BinaryReader.readStructMultiple(
      dataView,
      structs.subModel,
      bodyPart.modelIndex,
      bodyPart.numModels
 ...
 * @todo make shorter
 */
export const parseMeshes = (
  dataView: DataView,
  subModels: structs.SubModel[][]
): structs.Mesh[][][] =>
 ...
 */
export const parseVertBoneBuffer = (
  buffer: ArrayBuffer,
  subModels: structs.SubModel[][]
): Uint8Array[][] =>
  subModels.map((bodyPart) =>
    bodyPart.map((subModel) => new Uint8Array(buffer, subModel.vertInfoIndex, subModel.numVerts))
  )
...
export const parseTriangles = (
  ...
  meshes.map((bodyPart) =>
    bodyPart.map((subModel) =>
      subModel.map(
        (mesh) =>
          new Int16Array(buffer, mesh.triIndex, Math.floor((headerLength - mesh.triIndex) / 2))
      )
    )
  )
...
export const parseAnimValues = (
  ...
  const animValues = MultiArrayView.create(
    [sequences.length, numBones, AXLES_NUM, MAX_SRCBONES, 3],
    Int16Array
  )
    ...
    for (let j = 0; j < numBones; j++) {
      ...
      for (let axis = 0; axis < AXLES_NUM; axis++) {
        for (let v = 0; v < MAX_SRCBONES; v++) {
          const offset =
            animationIndex + animations[i][j].offset[axis + AXLES_NUM] + v * short.byteLength
...
export const parseModel = (modelBuffer: ArrayBuffer) => {
  ...
  // Body parts info
  const bodyParts: structs.BodyPart[] = parseBodyParts(
    dataView,
    header.bodyPartIndex,
    header.numBodyParts
  )
  // Submodels info
  ...
  const modelData = {
    ...
    /** Bone controllers */
    boneControllers: parseBoneControllers(
      dataView,
    ...
    /** Skins references */
    skinRef: parseSkinRef(dataView.buffer as ArrayBuffer, header.skinIndex, header.numSkinRef),
    ...
    vertices: parseVertices(dataView.buffer as ArrayBuffer, subModels),
    /** Bones vertices buffer. Path: vertBoneBuffer[bodyPartIndex][subModelIndex] */
    vertBoneBuffer: parseVertBoneBuffer(dataView.buffer as ArrayBuffer, subModels),
    /** Mesh triangles. Path: meshes[bodyPartIndex][subModelIndex][meshIndex] */
    triangles: parseTriangles(dataView.buffer as ArrayBuffer, meshes, header.length),
