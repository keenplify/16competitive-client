...
var developmentContentSecurityPolicy = "default-src 'self'; script-src 'self' 'unsafe-eval'; connect-src 'self' http: ws: https://mastodon.social; worker-src 'self' blob:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https: http:";
var developmentCsp = {
...
var electron_vite_config_default = defineConfig(({ mode }) => {
  ...
  const mainProcessEnv = Object.fromEntries(
    mainProcessEnvNames.flatMap((name) => {
      const value = process.env[name] || env[name] || (mode === "production" ? productionMainProcessEnv[name] : "");
