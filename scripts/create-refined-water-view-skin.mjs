...
const PATTERN_PATH = fileURLToPath(
  new URL('../resources/skin-patterns/desktop-pixel-water.png', import.meta.url)
)
const PAINTED_PARTS = {
...
function colorizeTexture(bmp, texture, pattern, patternScale) {
  ...
  if (width !== texture.width || height !== texture.height)
    throw new Error(`${texture.file} dimensions changed`)
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    luminance[i] =
      palette[offset] * 0.2126 + palette[offset + 1] * 0.7152 + palette[offset + 2] * 0.0722
  }
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
...
export async function createRefinedWaterViewSkin(
  sourceDirectory,
  outputDirectory,
  ...
  if (!Number.isFinite(patternScale) || patternScale <= 0)
    throw new Error('Pattern scale must be positive')
  if (!['matte-white', 'frosted-pearl', 'dark-navy-glass'].includes(material))
    throw new Error('Unsupported hardware material')
  ...
  if (
    manifest.format !== 'goldsrc-mdl-textures-v1' ||
  ...
  await writeFile(
    path.join(outputDirectory, 'textures.json'),
  ...
  for (const texture of manifest.textures) {
    if (
      ...
      path.basename(texture.file) !== texture.file
    ) {
    ...
    if (modelName.startsWith('v_') && texture.index < 3) {
      await copyFile(source, output)
      continue
    }
    const bmp = await readFile(source)
    const rgb =
      (modelName.startsWith('p_') || modelName.startsWith('w_')) &&
...
async function main() {
  ...
  if (!source || !output) {
    console.error(
      'Usage: node scripts/create-refined-water-view-skin.mjs <extracted-v-textures> <output-folder> [pattern.png] [pattern-scale] [material] [material-swatch.png]'
    )
    process.exitCode = 1
    return
  }
  const result = await createRefinedWaterViewSkin(
    source,
    output,
    pattern,
    scale === undefined ? 2.2 : Number(scale),
    material,
    swatch
  )
  console.log(
    `${result.modelName}: painted ${result.changed} textures in ${result.outputDirectory}`
  )
}
