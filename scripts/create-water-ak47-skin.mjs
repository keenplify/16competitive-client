...
const PATTERN_PATH = fileURLToPath(
  new URL('../resources/skin-patterns/water-caustics.png', import.meta.url)
)
...
export function decodeRgbPng(png) {
  if (!png.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])))
    throw new Error('Invalid pattern PNG')
  const idat = []
  let width = 0
  let height = 0
  let channels = 0
  for (let offset = 8; offset + 12 <= png.length;) {
    ...
    if (type === 'IHDR') {
      ...
      if (png[start + 8] !== 8 || !channels)
        throw new Error('Pattern must be 8-bit RGB or RGBA PNG')
    }
    if (type === 'IDAT') idat.push(png.subarray(start, start + size))
    offset = start + size + 4
    if (type === 'IEND') break
  }
  if (!width || !height || width * height > 16_000_000 || !idat.length)
    throw new Error('Invalid pattern PNG dimensions')
...
function waterColors(original, texture, pattern) {
  ...
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      ...
      if (
        texture.name.toLowerCase() === 'lower_body.bmp' &&
        x >= 124 &&
        x <= 230 &&
        y >= 29 &&
        y <= 70 &&
        ...
        for (let channel = 0; channel < 3; channel++)
          rgb[pixel * 3 + channel] = palette[old + channel]
        continue
      }
      const neighboring =
      ...
      for (let channel = 0; channel < 3; channel++) {
        const blended =
          pattern.pixels[source + channel] * relief * 0.58 + oldBlue[channel] * 0.42 + edge
...
export async function createWaterModelSkin(
  sourceDirectory,
  outputDirectory,
  ...
  const outputManifest = {
    ...manifest,
    textures: manifest.textures.map((t) => ({ ...t, flags: t.flags & ~0x20 }))
  }
  await writeFile(
...
async function main() {
  ...
  if (!source || !output) {
    console.error(
      'Usage: node scripts/create-water-ak47-skin.mjs <extracted-texture-folder> <water-skin-folder> [pattern.png]'
    )
    process.exitCode = 1
    return
  }
