...
const generateNotes = async (previousTag, commits) => {
  const fallback = commits.length > 0 ? commits.map((commit) => `- ${commit}`).join('\n') : '- Release maintenance.'
  let workspace
  try {
    ...
    execFileSync('codex', ['exec', '--model', 'gpt-5.6-luna', '--output-last-message', outputPath, prompt], {
      stdio: 'inherit'
    })
